...
- [x] **解决方案 nonce 范围验证** - 未验证 nonce 是否在预期范围内
  - 文件: `src/core/Miner.cpp`
  - 解决方案: 在 verifySolution() 中验证 nonce 在设备分配的范围内

- [ ] **设备健康监控** - 无法检测 GPU 热问题、内存损坏
  - 文件: `src/core/Miner.h`
  - 解决方案: 添加温度监控、错误模式检测 (需要 NVML/ADL)

- [x] **单设备故障隔离** - 单个 GPU 故障可能影响整个矿场
  - 文件: `src/core/Farm.cpp`
  - 解决方案: 添加 markMinerFailed(), recoverFailedMiners(), 隔离故障设备
...
- [x] **优雅关闭处理** - 关闭时可能丢失待提交的 share
  - 文件: `src/main.cpp`, `src/stratum/StratumClient.cpp`
  - 解决方案: 添加 gracefulDisconnect() 等待待处理请求完成

- [x] **工作缓存** - 工作接收中断时会停滞
  - 文件: `src/core/Farm.cpp`
  - 解决方案: 添加 m_previousWork, activateFallbackWork() 保留备用工作
...
- [x] **并行 GPU 初始化** - 大型矿场启动较慢
  - 文件: `src/core/Farm.cpp`
  - 解决方案: 使用 std::async 并行初始化多个 GPU

- [ ] **内核参数调优** - 针对不同 GPU 架构优化
  - 文件: `src/cuda/CUDAMiner.cpp`, `src/opencl/CLMiner.cpp`
  - 解决方案: 添加更多可配置参数

- [x] **版本协商** - 固定发送 "tosminer/1.0.0"，无能力协商
  - 文件: `src/Version.h`, `src/stratum/StratumClient.cpp`
  - 解决方案: 中央版本定义，存储池版本用于未来协商
...
- [x] 重复解决方案过滤 (nonce 去重)
- [x] Nonce 范围验证 (检测设备范围外的 nonce)
- [x] 单设备故障隔离 (markMinerFailed, recoverFailedMiners)
- [x] 优雅关闭处理 (gracefulDisconnect 等待 share 提交)
- [x] 工作缓存 (previousWork 作为备用)
- [x] 并行 GPU 初始化 (std::async 并行启动)
- [x] 版本协商 (Version.h 中央版本定义)
